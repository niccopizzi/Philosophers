...
/*   Updated: 2025/02/08 13:42:54 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	locked_printing(size_t begin, int phil_num, t_param_data *data,
		char *action)
{
	size_t	end;
	...
	printf(BOLD "%ld %d %s\n" RESET, (end - begin), phil_num, action);
	if (action[0] == 'd')
...
void	ft_usleep(size_t milliseconds, t_param_data *data)
{
	...
	while (get_time_in_milliseconds() - starting_milliseconds < milliseconds
		&& read_or_update_death(data, 'r') == false)
		usleep(100);
}
