...
/*   Updated: 2025/02/08 13:41:14 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < argc)
	{
		...
		while (argv[i][x] != 0)
		{
			if (argv[i][x] >= '0' && argv[i][x] <= '9')
				...
				return (printf(BOLD RED"Insert only numbers as args!\n"),
					false);
		}
		i++;
	}
...
bool	are_arguments_in_limits(int argc, char *argv[])
{
	...
	if (simple_atoi(argv[1]) > MAX_THREADS)
		return (printerr(PHIL_ERR), false);
	i = 1;
	while (i < argc)
	{
		usleep_time = simple_atoi(argv[i]);
		if (usleep_time == 0)
			return (printerr(ZERO_ERR), false);
		if (usleep_time > MAX_USLEEP)
			return (printerr(USLEEP_ERR), false);
		i++;
	}
	return (true);
}
...
t_param_data	*init_data_struct(int argc, char *argv[])
{
	...
	if (pthread_mutex_init(&data->bool_mutex, NULL) != 0)
		return (pthread_mutex_destroy(&data->update_mutex),
			pthread_mutex_destroy(&data->display_mutex), free(data), NULL);
	return (data);
}
